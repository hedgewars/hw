...
struct Engine<'a> {
    ...
    generate_preview:
        Symbol<'a, unsafe fn(engine_state: &mut EngineInstance, preview: &mut PreviewInfo)>,
    dispose_preview:
        Symbol<'a, unsafe fn(engine_state: &mut EngineInstance, preview: &mut PreviewInfo)>,
    ...
    read_ipc: Symbol<
        'a,
        unsafe fn(engine_state: &mut EngineInstance, buf: *mut u8, size: usize) -> usize,
    >,
}
...
fn resize_mono_preview(
    ...
    let h_offset: u32 = max(
        0,
        (PREVIEW_WIDTH as i64 / 2) - (in_width / SCALE_FACTOR / 2) as i64,
    ) as u32;
...
fn main() {
    ...
    unsafe {
        ...
        println!(
            "Hedgewars engine, protocol version {}",
        ...
        opts.optopt(
            "",
            "user-prefix",
            "Set the path to the custom data folder to find game content",
            "PATH_TO_FOLDER",
        );
        opts.optopt(
            "",
            "prefix",
            "Set the path to the system game data folder",
            "PATH_TO_FOLDER",
        );
        ...
        if matches.opt_present("landpreview") {
            let mut stream = TcpStream::connect(format!("127.0.0.1:{}", port))
                .expect("Failed to connect to IPC port. Feelsbadman.");
            ...
            stream
                ...
                .expect("IPC shutdown call failed");
