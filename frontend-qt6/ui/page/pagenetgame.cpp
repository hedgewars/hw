...
const int cutoffHeight = 688; /* Don't make this number below 605, or else it'll
                                 let the GameCFGWidget shrink too much before
                                 switching to tabbed mode. */
...
QLayout* PageNetGame::bodyLayoutDefinition() {
  ...
  leRoomName->setStyleSheet(QStringLiteral(
      "border-right: 0; padding-left: 4px; border-top-right-radius: 0px; "
  ...
  BtnUpdate->setStyleSheet(
      QStringLiteral("border-top-left-radius: 0px; border-bottom-left-radius: "
                     "0px; padding: auto 4px;"));
  ...
  lblRoomNameReadOnly->setSizePolicy(QSizePolicy::Expanding,
                                     QSizePolicy::Fixed);
  ...
  BtnMaster->setStyleSheet(
      QStringLiteral("QPushButton { padding: auto 4px; } QPushButton:pressed { "
                     "background-color: #ffcc00; border-color: #ffcc00; "
                     "border-bottom-left-radius: 0px; "
                     "border-bottom-right-radius: 0px; color: #11084A; }"));
  ...
  pNetTeamsWidget->setSizePolicy(QSizePolicy::Expanding,
  ...
  chatWidget->setShowFollow(
      false);  // don't show follow in nicks' context menus
...
QLayout* PageNetGame::footerLayoutDefinition() {
  ...
  BtnGo->setWhatsThis(
      tr("Turn on the lightbulb to show the other players when you're ready to "
         "fight"));
...
void PageNetGame::connectSignals() {
  connect(btnSetup, &QAbstractButton::clicked, this,
          &PageNetGame::SetupClicked);

  connect(BtnUpdate, &QAbstractButton::clicked, this,
          &PageNetGame::onUpdateClick);
  connect(leRoomName, &QLineEdit::returnPressed, this,
          &PageNetGame::onUpdateClick);

  connect(leRoomName, &QLineEdit::textChanged, this,
          &PageNetGame::onRoomNameEdited);
}
...
PageNetGame::PageNetGame(QWidget* parent) : AbstractPage(parent) {
  ...
  restrictUnregistered =
      new QAction(QAction::tr("Restrict Unregistered Players Join"), menu);
  ...
  if (height() < cutoffHeight) pGameCFG->setTabbed(true);
}
...
void PageNetGame::onRoomNameEdited() { BtnUpdate->setEnabled(true); }
