...
QLayout* PageNetServer::bodyLayoutDefinition() {
  ...
  BtnDefault->setWhatsThis(
      QPushButton::tr("Set the default server port for Hedgewars"));
  gbLayout->addWidget(BtnDefault, 1, 2);

  BtnShare = new QPushButton(gb);
  BtnShare->setText(
      QPushButton::tr("Invite your friends to your server in just 1 click!"));
  BtnShare->setWhatsThis(QPushButton::tr(
      "Click to copy your unique server URL to your clipboard. Send this link "
      "to your friends and they will be able to join you."));
  ...
  labelURL->setText(
      ...
      tr("Click here for details") + QStringLiteral("</a></div>"));
...
void PageNetServer::connectSignals() {
  connect(BtnDefault, &QAbstractButton::clicked, this,
          &PageNetServer::setDefaultPort);
...
void PageNetServer::setDefaultPort() { sbPort->setValue(NETGAME_DEFAULT_PORT); }

// This function assumes that the user wants to share their server while
// connected to the Internet and that he/she is using direct access (eg no
// NATs). To determine the IP we briefly connect to Hedgewars website and
// fallback to user intervention after 4 seconds of timeout.
void PageNetServer::copyUrl() {
  ...
  if (socket.waitForConnected(4000))
    ...
    address += QStringLiteral("<") + tr("Insert your address here") +
               QStringLiteral(">");
