...
QLayout *PageDataDownload::footerLayoutDefinition() {
  ...
  pbHome = addButton(QStringLiteral(":/res/home.png"), bottomLayout, 1, true,
                     Qt::AlignBottom);
  pbHome->setMinimumHeight(50);
  pbHome->setMinimumWidth(50);
  pbHome->setWhatsThis(tr("Load the start page"));

  pbOpenDir = addButton(QStringLiteral(":/res/folder.png"), bottomLayout, 2,
                        true, Qt::AlignBottom);
...
void PageDataDownload::connectSignals() {
  ...
  connect(pbOpenDir, &QAbstractButton::clicked, this,
          &PageDataDownload::openPackagesDir);
  connect(pbHome, &QAbstractButton::clicked, this,
          &PageDataDownload::fetchList);
}
...
PageDataDownload::PageDataDownload(QWidget *parent) : AbstractPage(parent) {
  ...
  //    fetchList();
  web->setHtml(QString("<center><h2>Hedgewars Downloadable Content</h2><br><br>"
                       "<i>%1</i></center>")
...
void PageDataDownload::request(const QUrl &url) {
  ...
  if (url.path().endsWith(QLatin1String(".hwp")) ||
      url.path().endsWith(QLatin1String(".zip"))) {
    ...
    connect(reply, &QNetworkReply::finished, this,
            &PageDataDownload::fileDownloaded);
    connect(reply, &QNetworkReply::downloadProgress, this,
            &PageDataDownload::downloadProgress);
    ...
    QNetworkReply *reply = manager->get(newRequest);
    connect(reply, &QNetworkReply::finished, this,
            &PageDataDownload::pageDownloaded);
  }
}
    ...
    if (reply->error() == QNetworkReply::NoError) {
      ...
      QString message =
          reply->error() == QNetworkReply::UnknownNetworkError
              ...
              : QString(tr("This feature requires an Internet connection, but "
                           "you don't appear to be online (error code: %1)."))
                    .arg(reply->error());
      web->setHtml(QString(html).arg(message));
    }
  } else {
    web->setHtml(
        QString(html).arg(tr("Internal error: Reply object is invalid.")));
  }
}
...
void PageDataDownload::fileDownloaded() {
  ...
  if (reply) {
    ...
    QString fileName =
        extractDir.filePath(QFileInfo(reply->url().path()).fileName());
...
void PageDataDownload::downloadProgress(qint64 bytesRecieved,
                                        qint64 bytesTotal) {
  QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
...
void PageDataDownload::openPackagesDir() {
  QString path =
      QDir::toNativeSeparators(cfgdir.absolutePath() + QStringLiteral("/Data"));
