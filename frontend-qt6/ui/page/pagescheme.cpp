...
QLayout* PageScheme::bodyLayoutDefinition() {
  ...
  gbGameModes->setStyleSheet(
      ".QGroupBox {"
      "background-color: #130f2c; background-image:url();"
      "}");
  gbBasicSettings->setStyleSheet(
      ".QGroupBox {"
      "background-color: #130f2c; background-image:url();"
      "}");
  ...
  TBW_disablegirders = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnDisableGirders@2x.png"));
  TBW_disablegirders->setWhatsThis(
      tr("Disable girders when generating random maps."));
  glGMLayout->addWidget(TBW_disablegirders, 0, 0, 1, 1);

  TBW_disablelandobjects = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnDisableLandObjects@2x.png"));
  TBW_disablelandobjects->setWhatsThis(
      tr("Disable land objects when generating random maps."));
  glGMLayout->addWidget(TBW_disablelandobjects, 0, 1, 1, 1);

  TBW_border = new ToggleButtonWidget(gbGameModes,
                                      QStringLiteral(":/res/btnBorder@2x.png"));
  TBW_border->setWhatsThis(
      tr("Add an indestructible border around the terrain"));
  glGMLayout->addWidget(TBW_border, 0, 2, 1, 1);

  TBW_bottomborder = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnBottomBorder@2x.png"));
  TBW_bottomborder->setWhatsThis(
      tr("Add an indestructible border along the bottom"));
  glGMLayout->addWidget(TBW_bottomborder, 0, 3, 1, 1);

  TBW_switchhog = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnSwitchHog@2x.png"));
  TBW_switchhog->setWhatsThis(
      tr("Select a hedgehog at the beginning of a turn"));
  glGMLayout->addWidget(TBW_switchhog, 0, 4, 1, 1);

  TBW_solid = new ToggleButtonWidget(gbGameModes,
                                     QStringLiteral(":/res/btnSolid@2x.png"));
  TBW_solid->setWhatsThis(tr("Land can not be destroyed by most weapons."));
  glGMLayout->addWidget(TBW_solid, 1, 0, 1, 1);

  TBW_lowGravity = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnLowGravity@2x.png"));
  TBW_lowGravity->setWhatsThis(tr("Lower gravity"));
  glGMLayout->addWidget(TBW_lowGravity, 1, 1, 1, 1);

  TBW_nowind = new ToggleButtonWidget(gbGameModes,
                                      QStringLiteral(":/res/btnNoWind@2x.png"));
  TBW_nowind->setWhatsThis(
      tr("You will not have to worry about wind anymore."));
  glGMLayout->addWidget(TBW_nowind, 1, 2, 1, 1);

  TBW_morewind = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnMoreWind@2x.png"));
  TBW_morewind->setWhatsThis(tr("Wind will affect almost everything."));
  glGMLayout->addWidget(TBW_morewind, 1, 3, 1, 1);

  TBW_artillery = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnArtillery@2x.png"));
  TBW_artillery->setWhatsThis(tr(
      "Your hogs are unable to move, put your artillery skills to the test"));
  glGMLayout->addWidget(TBW_artillery, 1, 4, 1, 1);

  TBW_vampiric = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnVampiric@2x.png"));
  TBW_vampiric->setWhatsThis(
      tr("Gain 80% of the damage you do back in health"));
  glGMLayout->addWidget(TBW_vampiric, 2, 0, 1, 1);

  TBW_karma = new ToggleButtonWidget(gbGameModes,
                                     QStringLiteral(":/res/btnKarma@2x.png"));
  TBW_karma->setWhatsThis(tr("Share your opponents pain, share their damage"));
  glGMLayout->addWidget(TBW_karma, 2, 1, 1, 1);

  TBW_resethealth = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnResetHealth@2x.png"));
  TBW_resethealth->setWhatsThis(
      tr("All (living) hedgehogs are fully restored at the end of turn"));
  glGMLayout->addWidget(TBW_resethealth, 2, 2, 1, 1);

  TBW_aisurvival = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnAISurvival@2x.png"));
  TBW_aisurvival->setWhatsThis(tr("AI respawns on death."));
  glGMLayout->addWidget(TBW_aisurvival, 2, 3, 1, 1);

  TBW_invulnerable = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnInvulnerable@2x.png"));
  TBW_invulnerable->setWhatsThis(tr("All hogs have a personal forcefield"));
  glGMLayout->addWidget(TBW_invulnerable, 2, 4, 1, 1);

  TBW_perhogammo = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnPerHogAmmo@2x.png"));
  TBW_perhogammo->setWhatsThis(
      tr("Each hedgehog has its own ammo. It does not share with the team."));
  glGMLayout->addWidget(TBW_perhogammo, 3, 0, 1, 1);

  TBW_sharedammo = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnSharedAmmo@2x.png"));
  TBW_sharedammo->setWhatsThis(
      tr("Ammo is shared between all teams that share a colour."));
  glGMLayout->addWidget(TBW_sharedammo, 3, 1, 1, 1);

  TBW_resetweps = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnResetWeps@2x.png"));
  TBW_resetweps->setWhatsThis(
      tr("Weapons are reset to starting values each turn."));
  glGMLayout->addWidget(TBW_resetweps, 3, 2, 1, 1);

  TBW_infattack = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnInfAttack@2x.png"));
  TBW_infattack->setWhatsThis(tr("Attacking does not end your turn."));
  glGMLayout->addWidget(TBW_infattack, 3, 3, 1, 1);

  TBW_laserSight = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnLaserSight@2x.png"));
  TBW_laserSight->setWhatsThis(tr("Assisted aiming with laser sight"));
  glGMLayout->addWidget(TBW_laserSight, 3, 4, 1, 1);

  TBW_randomorder = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnRandomOrder@2x.png"));
  TBW_randomorder->setWhatsThis(
      tr("Order of play is random instead of in room order."));
  glGMLayout->addWidget(TBW_randomorder, 4, 0, 1, 1);

  TBW_placehog = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnPlaceHog@2x.png"));
  TBW_placehog->setWhatsThis(
      tr("Take turns placing your hedgehogs before the start of play."));
  glGMLayout->addWidget(TBW_placehog, 4, 1, 1, 1);

  TBW_teamsDivide = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnTeamsDivide@2x.png"));
  TBW_teamsDivide->setWhatsThis(
      tr("Each clan starts in its own part of the terrain."));
  glGMLayout->addWidget(TBW_teamsDivide, 4, 2, 1, 1);

  TBW_tagteam = new ToggleButtonWidget(
      gbGameModes, QStringLiteral(":/res/btnTagTeam@2x.png"));
  TBW_tagteam->setWhatsThis(
      tr("Teams in each clan take successive turns sharing their turn time."));
  glGMLayout->addWidget(TBW_tagteam, 4, 3, 1, 1);

  TBW_king = new ToggleButtonWidget(gbGameModes,
                                    QStringLiteral(":/res/btnKing@2x.png"));
  ...
  //: Description of the game scheme setting “Damage Modifier”. “Knockback”
  //: means how much hedgehogs and objects get pushed by explosions and other
  //: forces
  ...
  QString wtSuddenDeath =
      tr("How many rounds have to be played before Sudden Death begins");
  QString wtWaterRise =
      tr("How much the water rises per turn while in Sudden Death. Set to 0 "
         "along with Sudden Death Health Decrease to disable Sudden Death.");
  QString wtHealthDecrease =
      tr("How much health hedgehogs lose per turn while in Sudden Death, down "
         "to 1 health. Set to 0 along with Sudden Death Water Rise to disable "
         "Sudden Death.");
  QString wtRopeModifier = tr("Maximum rope length in percent");
  QString wtHealthCrates =
      tr("Likelihood of a dropped crate being a health crate. All other crates "
         "will be weapon or utility crates.");
  ...
  QString wtMinesTime = tr(
      "Detonation timer of mines. The random timer lies between 0 and 5 "
      "seconds. The timer of air mines will be a quarter of the mines timer.");
  QString wtMines =
      tr("Average number of mines to be placed a medium-sized island map. This "
         "number will be scaled for other maps.");
  QString wtMineDuds =
      tr("Likelihood of a mine being a dud. Does not affect mines placed by "
         "hedgehogs.");
  QString wtExplosives =
      tr("Average number of barrels to be placed a medium-sized island map. "
         "This number will be scaled for other maps.");
  QString wtAirMines =
      tr("Average number of air mines to be placed a medium-sized island map. "
         "This number will be scaled for other maps.");
  QString wtSentries =
      tr("Average number of sentry bots to be placed on a medium-sized island "
         "map. This number will be scaled for other maps.");
  ...
  QString wtScriptParam =
      tr("Additional parameter to configure game styles. The meaning depends "
         "on the used style, refer to the documentation. When in doubt, leave "
         "it empty.");
  ...
  auto val = new QRegularExpressionValidator(rx, LE_name);
...
void PageScheme::deleteRow() {
  ...
  int numberOfDefaultSchemes =
      ((GameSchemeModel*)mapper->model())->numberOfDefaultSchemes;
  if (selectScheme->currentIndex() < numberOfDefaultSchemes) {
    ...
    deniedMsg.setText(QMessageBox::tr("Cannot delete default scheme '%1'!")
                          .arg(selectScheme->currentText()));
    ...
    reallyDeleteMsg.setText(
        QMessageBox::tr("Do you really want to delete the game scheme '%1'?")
            .arg(selectScheme->currentText()));
...
void PageScheme::checkDupe() {
  ...
  if (dupe) {
    ...
    int i = 2;
    while (
        model->hasScheme(newName = tr("%1 (%2)").arg(name).arg(i++), except)) {
      if (i > 1000) {
        return;
      }
    }
    ...
    dupeMsg.setText(
        QMessageBox::tr("A scheme with the name '%1' already exists. Your "
                        "scheme has been renamed to '%2'.")
            .arg(name, newName));
