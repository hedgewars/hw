...
SeedPrompt::SeedPrompt(QWidget* parent, const QString& seed, bool editable)
    : QDialog(parent) {
  setModal(true);
  ...
  // Label
  QLabel* label =
      new QLabel(tr("The map seed is the basis for all random values generated "
                    "by the game."));
  ...
  if (editable) {
    ...
    connect(btnClose, &QAbstractButton::clicked, this, &QDialog::reject);
  ...
  connect(this, &QDialog::accepted, this, &SeedPrompt::setSeed);
}

void SeedPrompt::setSeed() { Q_EMIT seedSelected(editBox->text()); }
