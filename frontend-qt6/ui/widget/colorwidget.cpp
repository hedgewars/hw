...
void ColorWidget::setColor(int color)
{
    ...
    setStyleSheet(QStringLiteral("* { border: 2px solid #ffcc00; border-radius: 8px; background: %1 } :disabled { border-color: #a0a0a0; } :hover { border-color: #ffff00; }").arg(item->data().value<QColor>().name()));
    /*
    QPalette p = palette();
