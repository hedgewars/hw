    ...
    , seedRegexp(QStringLiteral("\\{[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\\}"))
{
  ...
  mapContainerFree->setFixedSize(pMapContainer->width(),
                                 pMapContainer->height());
  ...
  OptionsInnerContainer->setFixedSize(optionsContainerFree->width(),
                                      optionsContainerFree->height());
  OptionsInnerContainer->setSizePolicy(QSizePolicy::Expanding,
  ...
  SchemeWidgetLayout->setContentsMargins({});
  ...
  bindEntries->setStyleSheet(
      "QCheckBox::indicator:checked:enabled    { image: "
      "url(\":/res/lock.png\"); }"
      "QCheckBox::indicator:checked:disabled   { image: "
      "url(\":/res/lock_disabled.png\"); }"
      "QCheckBox::indicator:unchecked:enabled  { image: "
      "url(\":/res/unlock.png\");   }"
      "QCheckBox::indicator:unchecked:disabled { image: "
...
void GameCFGWidget::setNetAmmo(const QString& name, const QString& ammo)
{
  ...
  if (illegal) {
    ...
    illegalMsg.setText(
        QMessageBox::tr("Cannot use the weapon scheme '%1'!").arg(name));
