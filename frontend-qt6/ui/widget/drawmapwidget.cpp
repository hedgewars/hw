...
DrawMapWidget::DrawMapWidget(QWidget *parent)
    : QWidget(parent), ui(new Ui::DrawMapWidget) {
  ui->setupUi(this);

  m_scene = 0;
}

DrawMapWidget::~DrawMapWidget() { delete ui; }
...
void DrawMapWidget::setScene(DrawMapScene *scene) {
  ...
  connect(scene, &DrawMapScene::brushSizeChanged, this,
          &DrawMapWidget::brushSizeChanged_slot);
}
...
void DrawMapWidget::save(const QString &fileName) {
  if (m_scene) {
    ...
    if (!file.open(QIODevice::WriteOnly)) {
      ...
      errorMsg.setText(
          QMessageBox::tr("Cannot open '%1' for writing").arg(fileName));
...
DrawMapView::DrawMapView(QWidget *parent) : QGraphicsView(parent) {
...
void DrawMapView::enterEvent(QEnterEvent *event) {
  if (m_scene) m_scene->showCursor();

  QGraphicsView::enterEvent(event);
}

void DrawMapView::leaveEvent(QEvent *event) {
  if (m_scene) m_scene->hideCursor();
