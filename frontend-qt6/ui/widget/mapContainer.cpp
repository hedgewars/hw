...
HWMapContainer::HWMapContainer(QWidget *parent)
  ...
  mainLayout.setContentsMargins(
      HWApplication::style()->pixelMetric(QStyle::PM_LayoutLeftMargin), 10,
  ...
  btnSeed->setWhatsThis(
      tr("View and edit the seed, the source of randomness in the game"));
  btnSeed->setStyleSheet(QStringLiteral("padding: 5px;"));
  btnSeed->setFixedHeight(cType->height());
  connect(btnSeed, &QAbstractButton::clicked, this,
          &HWMapContainer::showSeedPrompt);
...
void HWMapContainer::setImage(const QPixmap &newImage,
                              const QLinearGradient &linearGrad,
                              bool showHHLimit) {
...
void HWMapContainer::setHHLimit(int newHHLimit) { hhLimit = newHHLimit; }
...
void HWMapContainer::addInfoToPreview(const QPixmap &image,
  ...
  if (drawHHLimit) {
    ...
    p.drawRect(finalImage.rect().width() - hhSmall.rect().width() - 28, 3, 40,
               20);

    QString text =
        (hhLimit > 0) ? QString::number(hhLimit) : QStringLiteral("?");
    p.drawText(finalImage.rect().width() - hhSmall.rect().width() - 14 -
                   (hhLimit > 9 ? 10 : 0),
               18, text);
    p.drawPixmap(finalImage.rect().width() - hhSmall.rect().width() - 5, 5,
                 hhSmall.rect().width(), hhSmall.rect().height(), hhSmall);
  }
...
void HWMapContainer::askForGeneratedPreview() {
  ...
  connect(pMap, &QObject::destroyed, this,
          &HWMapContainer::onPreviewMapDestroyed);
  pMap->getImage(m_seed, getTemplateFilter(), get_mapgen(), getMazeSize(),
                 getDrawnMapData(), m_script, m_scriptparam, m_mapFeatureSize);
...
void HWMapContainer::previewClicked() {
  if (isMaster())  // should only perform these if master, but disabling the
                   // button when not, causes an unattractive preview.
    switch (m_mapInfo.type) {
      case MapModel::HandDrawnMap:
...
QString HWMapContainer::getCurrentSeed() const { return m_seed; }
...
QString HWMapContainer::getCurrentTheme() const { return (m_theme); }

bool HWMapContainer::getCurrentIsMission() const {
  return (m_mapInfo.type == MapModel::MissionMap);
}

int HWMapContainer::getCurrentHHLimit() const { return hhLimit; }

QString HWMapContainer::getCurrentScheme() const { return (m_mapInfo.scheme); }
...
quint32 HWMapContainer::getFeatureSize() const { return m_mapFeatureSize; }

void HWMapContainer::resizeEvent(QResizeEvent *event) { Q_UNUSED(event); }

void HWMapContainer::intSetSeed(const QString &seed) { m_seed = seed; }
...
void HWMapContainer::intSetMap(const QString &map) {
  if (map == QLatin1String("+rnd+")) {
    ...
    changeMapType(MapModel::StaticMap,
                  m_staticMapModel->index(m_staticMapModel->findMap(map), 0));
  } else if (m_missionMapModel->mapExists(map)) {
    m_missingMap = false;
    changeMapType(MapModel::MissionMap,
                  m_missionMapModel->index(m_missionMapModel->findMap(map), 0));
  } else {
...
void HWMapContainer::setTheme(const QString &theme) {
  QModelIndexList mdl = m_themeModel->match(m_themeModel->index(0),
                                            ThemeModel::ActualNameRole, theme);
  ...
  switch (m_mapInfo.type) {
    ...
    case MapModel::MissionMap:
      if (m_withoutDLC) {
        ...
        missionMapChanged(
            mmodel->mapToSource(mmodel->index(rand() % mmodel->rowCount(), 0)));
      } else
        missionMapChanged(m_missionMapModel->index(
            rand() % m_missionMapModel->rowCount(), 0));
      break;
    case MapModel::StaticMap:
      if (m_withoutDLC) {
        ...
        staticMapChanged(
            m_staticMapModel->index(rand() % m_staticMapModel->rowCount(), 0));
      break;
    default:
      break;
  }
...
void HWMapContainer::setTemplateFilter(int filter) {
  ...
  if (m_mapInfo.type == MapModel::GeneratedMap) updatePreview();
}

MapGenerator HWMapContainer::get_mapgen(void) const { return mapgen; }

int HWMapContainer::getMazeSize(void) const { return mazeStyles->currentRow(); }
...
void HWMapContainer::setMazeSize(int size) {
  intSetMazeSize(size);
  if (m_mapInfo.type == MapModel::GeneratedMaze) updatePreview();
}
...
void HWMapContainer::intSetMapgen(MapGenerator m) {
  if (mapgen != m) {
    ...
    if (f) changeMapType(m_mapInfo.type, QModelIndex());
  }
}

void HWMapContainer::setMapgen(MapGenerator m) {
  intSetMapgen(m);
  if (m != MAPGEN_MAP) updatePreview();
}
...
QByteArray HWMapContainer::getDrawnMapData() { return drawMapScene.encode(); }
...
DrawMapScene *HWMapContainer::getDrawMapScene() { return &drawMapScene; }
...
void HWMapContainer::setAllMapParameters(const QString &map, MapGenerator m,
                                         int mazesize, const QString &seed,
                                         int tmpl, int featureSize) {
  intSetMapgen(m);
...
void HWMapContainer::updateModelViews() {
  ...
  if (!m_theme.isNull() && !m_theme.isEmpty()) {
    QModelIndexList mdl =
        m_themeModel->match(m_themeModel->index(0), Qt::DisplayRole, m_theme);
    if (mdl.size() > 0)
      updateTheme(mdl.at(0));
    else
      setRandomTheme();
...
void HWMapContainer::updateHelpTexts(MapModel::MapType type) {
  ...
  QString mfsComplex =
      QString(tr("Adjust the complexity of the generated map"));
...
void HWMapContainer::changeMapType(MapModel::MapType type,
                                   const QModelIndex &newMap) {
  staticMapList->hide();
  missionMapList->hide();
  teMapName->hide();
  lblMapList->hide();
  ...
  switch (type) {
    ...
    case MapModel::MissionMap:
      ...
      mapgen = MAPGEN_MAP;
      missionMapChanged(newMap.isValid() ? newMap
                                         : missionMapList->currentIndex());
    ...
    case MapModel::StaticMap:
      setupStaticMapsView();
      mapgen = MAPGEN_MAP;
      staticMapChanged(newMap.isValid() ? newMap
                                        : staticMapList->currentIndex());
...
void HWMapContainer::intSetFeatureSize(int val) {
  ...
  updateHelpTexts(
      (MapModel::MapType)cType->itemData(cType->currentIndex()).toInt());
...
void HWMapContainer::showThemePrompt() {
  ...
  int theme =
      prompt.exec() - 1;  // Since 0 means canceled, so all indexes are +1'd
  if (theme < 0) return;
...
void HWMapContainer::updateTheme(const QModelIndex &current) {
  ...
  QString themeLabel =
      tr("Theme: %1").arg(current.data(Qt::DisplayRole).toString());
...
void HWMapContainer::mapChanged(const QModelIndex &map, int type,
  ...
  Q_ASSERT(
      map.data(Qt::UserRole + 1)
          .canConvert<MapModel::MapInfo>());  // Houston, we have a problem.
...
void HWMapContainer::loadDrawing() {
  ...
  if (!f.open(QIODevice::ReadOnly)) {
    ...
    errorMsg.setText(
        QMessageBox::tr("Cannot open '%1' for reading").arg(fileName));
...
void HWMapContainer::showSeedPrompt() {
  SeedPrompt prompt(parentWidget()->parentWidget(), getCurrentSeed(),
                    isMaster());
  connect(&prompt, &SeedPrompt::seedSelected, this,
          &HWMapContainer::setNewSeed);
  prompt.exec();
}

bool HWMapContainer::isMaster() { return m_master; }
  ...
  if (master) {
    ...
    if (m_missingMap) {
      ...
      if (m_mapInfo.type == MapModel::MissionMap) {
        missionMapList->selectionModel()->setCurrentIndex(
            m_missionMapModel->index(0, 0),
            QItemSelectionModel::Clear | QItemSelectionModel::SelectCurrent);
      } else {
        if (m_mapInfo.type != MapModel::StaticMap) {
          changeMapType(MapModel::StaticMap);
        }
        staticMapList->selectionModel()->setCurrentIndex(
            m_staticMapModel->index(0, 0),
            QItemSelectionModel::Clear | QItemSelectionModel::SelectCurrent);
      }
    } else {
...
void HWMapContainer::setupMissionMapsView(const QString &initialMap) {
  ...
  connect(missionSelectionModel, &QItemSelectionModel::currentRowChanged, this,
          &HWMapContainer::missionMapChanged);
  int m = 0;
  if (!initialMap.isNull()) m = m_missionMapModel->findMap(initialMap);
  missionSelectionModel->setCurrentIndex(
      m_missionMapModel->index(m, 0),
      QItemSelectionModel::Clear | QItemSelectionModel::SelectCurrent);
}
...
void HWMapContainer::setupStaticMapsView(const QString &initialMap) {
  ...
  connect(staticSelectionModel, &QItemSelectionModel::currentRowChanged, this,
          &HWMapContainer::staticMapChanged);
  int m = 0;
  if (!initialMap.isNull()) m = m_staticMapModel->findMap(initialMap);
  staticSelectionModel->setCurrentIndex(
      m_staticMapModel->index(m, 0),
      QItemSelectionModel::Clear | QItemSelectionModel::SelectCurrent);
}
...
void HWMapContainer::setMapNameLabel(QString mapName, bool validMap) {
  ...
  if (validMap)
    ...
    teMapName->setStyleSheet(
        QStringLiteral("background-color: transparent; color: #b50000;"));
}
