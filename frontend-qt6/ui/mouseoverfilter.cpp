...
MouseOverFilter::MouseOverFilter(QObject *parent) : QObject(parent) {}
  ...
  if (event->type() == QEvent::Enter) {
    ...
    if (button || textfield || checkbox || droplist || slider || tab ||
      ...
      SDLInteraction::instance().playSoundFile(
          QStringLiteral("/Sounds/steps.ogg"));
    }
    ...
    if (abstractpage->getDefaultDescription() != NULL) {
      abstractpage->setButtonDescription(
          *abstractpage->getDefaultDescription());
    } else
...
void MouseOverFilter::setUi(Ui_HWForm *uiForm) { ui = uiForm; }
