...
// I started handing this down to each place it touches, but it was getting
// ridiculous and this one flag does not warrant a static class
...
HWForm::HWForm(QWidget *parent, const QString &styleSheet)
  ...
  connect(ui.pageMultiplayer->BtnStartMPGame, &QAbstractButton::clicked, this,
          &HWForm::StartMPGame);
  connect(ui.pageMultiplayer->teamsSelect, &TeamSelWidget::setEnabledGameStart,
  ...
  connect(ui.pageNetGame->pNetTeamsWidget, &TeamSelWidget::setEnabledGameStart,
          ui.pageNetGame->BtnStart, &QWidget::setEnabled);
  ...
  connect(ui.pageSelectWeapon->pWeapons, &SelWeaponWidget::weaponsDeleted, this,
          &HWForm::DeleteWeapons);
  connect(ui.pageSelectWeapon->pWeapons, &SelWeaponWidget::weaponsAdded, this,
          &HWForm::AddWeapons);
  connect(ui.pageSelectWeapon->pWeapons, &SelWeaponWidget::weaponsEdited, this,
          &HWForm::EditWeapons);
  ...
  gameSchemeModel = new GameSchemeModel(
      this, cfgdir.absolutePath() + QStringLiteral("/Schemes/Game"));
...
void HWForm::UpdateWeapons() {
  ...
  for (QVector<QComboBox *>::iterator it = combos.begin(); it != combos.end();
    ...
    for (int i = 0; i < names.size(); ++i)
      (*it)->addItem(names[i],
                     ui.pageSelectWeapon->pWeapons->getWeaponsString(names[i]));
...
void HWForm::UpdateTeamsLists() {
  ...
  if (teamslist.empty()) {
    ...
    // But we exclude DLC graves and forts to not have desyncing teams by
    // default
...
void HWForm::GoToEditWeapons() {
  ui.pageSelectWeapon->selectWeaponSet->setCurrentIndex(
      ui.pageOptions->WeaponsName->currentIndex());
  GoToPage(ID_PAGE_SELECTWEAPON);
}
...
void HWForm::GoToEditScheme() {
  ui.pageScheme->selectScheme->setCurrentIndex(
      ui.pageOptions->SchemesName->currentIndex());
  GoToPage(ID_PAGE_SCHEME);
}
...
void HWForm::GoToVideos() { GoToPage(ID_PAGE_VIDEOS); }

void HWForm::GoToTraining() { GoToPage(ID_PAGE_TRAINING); }

// TODO: maybe find a better place for this?
QString HWForm::stringifyPageId(quint32 id) {
  QString pageName;
  switch (id) {
    case ID_PAGE_SETUP_TEAM:
      pageName = QStringLiteral("PAGE_SETUP_TEAM");
      break;
    case ID_PAGE_SETUP:
      pageName = QStringLiteral("PAGE_SETUP");
      break;
    case ID_PAGE_MULTIPLAYER:
      pageName = QStringLiteral("PAGE_MULTIPLAYER");
      break;
    case ID_PAGE_DEMOS:
      pageName = QStringLiteral("PAGE_DEMOS");
      break;
    case ID_PAGE_NET:
      pageName = QStringLiteral("PAGE_NET");
      break;
    case ID_PAGE_NETGAME:
      pageName = QStringLiteral("PAGE_NETGAME");
      break;
    case ID_PAGE_INFO:
      pageName = QStringLiteral("PAGE_INFO");
      break;
    case ID_PAGE_MAIN:
      pageName = QStringLiteral("PAGE_MAIN");
      break;
    case ID_PAGE_GAMESTATS:
      pageName = QStringLiteral("PAGE_GAMESTATS");
      break;
    case ID_PAGE_SINGLEPLAYER:
      pageName = QStringLiteral("PAGE_SINGLEPLAYER");
      break;
    case ID_PAGE_TRAINING:
      pageName = QStringLiteral("PAGE_TRAINING");
      break;
    case ID_PAGE_SELECTWEAPON:
      pageName = QStringLiteral("PAGE_SELECTWEAPON");
      break;
    case ID_PAGE_NETSERVER:
      pageName = QStringLiteral("PAGE_NETSERVER");
      break;
    case ID_PAGE_INGAME:
      pageName = QStringLiteral("PAGE_INGAME");
      break;
    case ID_PAGE_ROOMSLIST:
      pageName = QStringLiteral("PAGE_ROOMSLIST");
      break;
    case ID_PAGE_CONNECTING:
      pageName = QStringLiteral("PAGE_CONNECTING");
      break;
    case ID_PAGE_SCHEME:
      pageName = QStringLiteral("PAGE_SCHEME");
      break;
    case ID_PAGE_ADMIN:
      pageName = QStringLiteral("PAGE_ADMIN");
      break;
    case ID_PAGE_CAMPAIGN:
      pageName = QStringLiteral("PAGE_CAMPAIGN");
      break;
    case ID_PAGE_DRAWMAP:
      pageName = QStringLiteral("PAGE_DRAWMAP");
      break;
    case ID_PAGE_DATADOWNLOAD:
      pageName = QStringLiteral("PAGE_DATADOWNLOAD");
      break;
    case ID_PAGE_VIDEOS:
      pageName = QStringLiteral("PAGE_VIDEOS");
      break;
    case MAX_PAGE:
      pageName = QStringLiteral("MAX_PAGE");
      break;
    default:
      pageName = QStringLiteral("UNKNOWN_PAGE");
      break;
  }
  return pageName;
}
...
void HWForm::OnPageShown(quint8 id, quint8 lastid) {
#ifdef QT_DEBUG
  qDebug("Leaving %s, entering %s", qPrintable(stringifyPageId(lastid)),
         qPrintable(stringifyPageId(id)));
#endif
  ...
  if (id == ID_PAGE_ROOMSLIST) {
    if (hwnet && game &&
        game->gameState == gsStarted)  // abnormal exit - kick or room
                                       // destruction - send kills.
    {
  ...
  if (id == ID_PAGE_MULTIPLAYER || id == ID_PAGE_NETGAME) {
    ...
    for (QStringList::iterator it = tmNames.begin(); it != tmNames.end();
         ++it) {
      HWTeam team(*it);
      team.loadFromFile();
    ...
    if (lastid == ID_PAGE_SETUP || lastid == ID_PAGE_DRAWMAP)  // _TEAM
    {
      if (editedTeam) {
        curTeamSelWidget->addTeam(*editedTeam);
               ...
               lastid != ID_PAGE_SCHEME && lastid != ID_PAGE_SELECTWEAPON) {
...
void HWForm::GoToPage(int id) {
  ...
  /* if (id == ID_PAGE_DRAWMAP || id == ID_PAGE_GAMESTATS)
       stopAnim = true;
   This were disabled due to broken flake animations.  I believe the more
   general problems w/ opacity that forced its disable makes blocking these
   unnecessary.
  */
  ...
  // if (!stopAnim)
  {
...
#ifdef false
    coeff = 2;
    QGraphicsOpacityEffect *effectNew =
        new QGraphicsOpacityEffect(ui.Pages->widget(id));
    ui.Pages->widget(id)->setGraphicsEffect(effectNew);

    QGraphicsOpacityEffect *effectLast =
        new QGraphicsOpacityEffect(ui.Pages->widget(lastid));
    ...
    /* this is for the situation when the animation below is interrupted by a
     * new animation.  For some reason, finished is not being fired */
    for (int i = 0; i < MAX_PAGE; i++)
      if (i != id && i != lastid) ui.Pages->widget(i)->hide();
  }
#endif
}
...
void HWForm::GoBack() {
  ...
  if (curid == ID_PAGE_MAIN) {
    ...
    if (!ui.pageVideos->tryQuit(this)) return;
    stopAnim = true;
    exit();
  }
  ...
  if (curid == ID_PAGE_CAMPAIGN)
    config->setValue(QStringLiteral("frontend/lastSingleplayerTeam"),
                     ui.pageCampaign->CBTeam->currentText());
  if (curid == ID_PAGE_TRAINING)
    config->setValue(QStringLiteral("frontend/lastSingleplayerTeam"),
                     ui.pageTraining->CBTeam->currentText());

  if (curid == ID_PAGE_ROOMSLIST || curid == ID_PAGE_CONNECTING)
    NetDisconnect();
  if (curid == ID_PAGE_NETGAME && hwnet && hwnet->isInRoom()) hwnet->partRoom();
  // need to work on this, can cause invalid state for admin quit trying to
  // prevent bad state message on kick
  // if (curid == ID_PAGE_NETGAME && (!game || game->gameState != gsStarted))
  // hwnet->partRoom();

  if (curid == ID_PAGE_SCHEME) gameSchemeModel->Save();
  ...
  if (curid != 0 && !stopAnim) {
    ...
    QGraphicsOpacityEffect *effectLast =
        new QGraphicsOpacityEffect(ui.Pages->widget(curid));
  ...
  if (stopAnim) ui.Pages->widget(curid)->hide();
...
void HWForm::OpenSnapshotFolder() {
  QString path = QDir::toNativeSeparators(cfgdir.absolutePath() +
                                          QStringLiteral("/Screenshots"));
  QDesktopServices::openUrl(QUrl(QStringLiteral("file:///") + path));
}

void HWForm::btnExitPressed() { eggTimer.start(); }
...
void HWForm::IntermediateSetup() {
  ...
  // UpdateTeamsLists(&tmnames); // FIXME: still need more work if teamname is
  // updated while configuring
  UpdateTeamsLists();
...
void HWForm::NewTeam() {
  ...
  if (teamslist.contains(teamName)) {
    ...
    int i = 2;
    while (
        teamslist.contains(teamName = QLineEdit::tr("unnamed (%1)").arg(i++)));
  }
...
void HWForm::DeleteScheme() {
  ui.pageScheme->selectScheme->setCurrentIndex(
      ui.pageOptions->SchemesName->currentIndex());
  if (ui.pageOptions->SchemesName->currentIndex() <
      gameSchemeModel->numberOfDefaultSchemes) {
    MessageDialog::ShowErrorMessage(
        QMessageBox::tr("Cannot delete default scheme '%1'!")
            .arg(ui.pageOptions->SchemesName->currentText()),
        this);
  } else {
...
void HWForm::PlayDemo() {
  ...
  if (!curritem) {
    MessageDialog::ShowErrorMessage(
        QMessageBox::tr("Please select a record from the list"), this);
    return;
  }
  CreateGame(0, 0, 0);
  game->PlayDemo(curritem->data(Qt::UserRole).toString(),
                 ui.pagePlayDemo->isSave());
}
...
void HWForm::NetConnectServer(const QString &host, quint16 port, bool useTls) {
  ...
  _NetConnect(host, port, useTls,
              ui.pageOptions->editNetNick->text().trimmed());
}
...
void HWForm::NetPassword(const QString &nick) {
  ...
  if (temphash.isEmpty() &&
      hash.isEmpty()) {  // If the user enters a registered nick with no
                         // password, sends a bogus hash
...
void HWForm::NetNickRegistered(const QString &nick) {
  ...
  if (hash.isEmpty()) {
    if (temphash.isEmpty()) {  // If the user enters a registered nick with no
                               // password
      ...
      while (suppliedpass.isEmpty()) {
        ...
        nickRegedDialog.setLabelText(
            tr("This nick is registered, and you haven't specified a "
               "password.\n\nIf this nick isn't yours, please register your "
               "own nick at www.hedgewars.org\n\nPassword:"));
        ...
        temphash = QCryptographicHash::hash(suppliedpass.toUtf8(),
                                            QCryptographicHash::Md5)
                       .toHex();
...
void HWForm::NetNickTaken(const QString &nick) {
  bool ok = false;
  QString newNick =
      QInputDialog::getText(this, tr("Nickname"),
                            tr("Someone already uses your nickname %1 on the "
  ...
  if (!ok || newNick.isEmpty()) {
    ...
    bool retry = RetryDialog(tr("Hedgewars - Empty nickname"),
                             tr("No nickname supplied."));
    GoBack();
    if (retry && hwnet) {
  ...
  if (hwnet) hwnet->NewNick(newNick);
...
void HWForm::NetAuthFailed() {
  // Set the password blank if case the user tries to join and enter their
  // password again
  config->clearTempHash();

  // Try to login again
  bool retry = RetryDialog(tr("Hedgewars - Wrong password"),
                           tr("You entered a wrong password."));
  GoBack();
...
void HWForm::askRoomPassword() {
  QString password = QInputDialog::getText(
      this, tr("Room password"),
      tr("The room is protected with password.\nPlease, enter the password:"));
  if (hwnet && !password.isEmpty()) hwnet->roomPasswordEntered(password);
}
...
int HWForm::AskForNickAndPwd(void) {
  ...
  do {
    ...
    if (nickname.isEmpty() || hash.isEmpty()) {
      ...
      // make the "new account" button dialog open a browser with the
      // registration page
      connect(pwDialog->pbNewAccount, &QAbstractButton::clicked, this,
              &HWForm::openRegistrationPage);
      pwDialog->cbSave->setChecked(
          config->value("net/savepassword", true).toBool());
      ...
      if (nickname.isEmpty()) {
        int retry = RetryDialog(tr("Hedgewars - Empty nickname"),
                                tr("No nickname supplied."));
        GoBack();
        if (retry) {
          if (hwnet->m_private_game) {
      ...
      nickname.isEmpty() ||
      (hash.isEmpty() &&
       temphash.isEmpty()));  // while a nickname, or both hashes are missing
...
void HWForm::NetConnect() {
  ...
  if (hpd->exec() == QDialog::Accepted) {
    config->SaveOptions();
    netHost = QString(hpd->leHost->text());
...
void HWForm::NetStartServer() {
  ...
  pRegisterServer =
      new HWNetUdpServer(0, ui.pageNetServer->leServerDescr->text(),
                         ui.pageNetServer->sbPort->value());
}

void HWForm::AsyncNetServerStart() {
  NetConnectServer(QStringLiteral("localhost"), pnetserver->getRunningPort(),
                   false);
}
...
void HWForm::NetDisconnect() {
  ...
  if (hwnet) hwnet->Disconnect();
}

void HWForm::ForcedDisconnect(const QString &reason) {
  if (reason ==
      QLatin1String("Reconnected too fast")) {  // TODO: this is a hack, which
                                                // should be remade
    bool retry = RetryDialog(tr("Hedgewars - Connection error"),
                             tr("You reconnected too fast.\nPlease wait a few "
  ...
  if (hwnet && (reason != QLatin1String("bye"))) {
    QString errorStr =
        QMessageBox::tr("The connection to the server is lost.") +
        (reason.isEmpty() ? QLatin1String("")
                          : (QStringLiteral("\n\n") + HWNewNet::tr("Reason:") +
                             QStringLiteral("\n") + reason));
...
void HWForm::NetRedirected(quint16 port) {
  ...
  questionMsg.setText(
      QMessageBox::tr("This server supports secure connections on port "
...
void HWForm::NetConnected() { GoToPage(ID_PAGE_ROOMSLIST); }
...
void HWForm::StartMPGame() {
  QString ammo;
  ammo =
      ui.pageMultiplayer->gameCFG->WeaponsName
          ->itemData(ui.pageMultiplayer->gameCFG->WeaponsName->currentIndex())
          .toString();
...
void HWForm::GameStateChanged(GameState gameState) {
  ...
  switch (gameState) {
    case gsStarted: {
      ...
      if (!hwnet || (!hwnet->isRoomChief() || !hwnet->isInRoom()))
    ...
    case gsFinished: {
      ...
      if (hwnet) {
        if (!game || !game->netSuspend) hwnet->gameFinished(true);
    ...
    default: {
      ...
      if (id == ID_PAGE_INGAME ||
          ...
          (hwnet && hwnet->isRoomChief() && hwnet->isInRoom() &&
           (gameState == gsInterrupted || gameState == gsStopped ||
            gameState == gsDestroyed || gameState == gsHalted))) {
        if (id == ID_PAGE_INGAME) GoBack();
        Music(ui.pageOptions->CBFrontendMusic->isChecked());
        if (wBackground) wBackground->startAnimation();
...
void HWForm::DemoPresenceChanged(bool hasDemo) { demoIsPresent = hasDemo; }
...
void HWForm::GetRecord(RecordType type, const QByteArray &record) {
  if (type != rtNeither) {
    ...
    QString recordFileName = config->appendDateTimeToRecordName()
                                 ? QDateTime::currentDateTime().toString(
                                       QStringLiteral("yyyy-MM-dd_hh-mm"))
                                 : QStringLiteral("LastRound");

    recordFileName += QStringLiteral("_") + cRevisionString +
                      QStringLiteral("-") + cHashString;

    if (type == rtDemo) {
      demo.replace(QByteArray("\x02TL"), QByteArray("\x02TD"));
      demo.replace(QByteArray("\x02TN"), QByteArray("\x02TD"));
      demo.replace(QByteArray("\x02TS"), QByteArray("\x02TD"));
      filename = cfgdir.absolutePath() + QStringLiteral("/Demos/") +
                 recordFileName + QStringLiteral(".") + cProtoVer +
                 QStringLiteral(".hwd");
      m_lastDemo = demo;
    } else {
      demo.replace(QByteArray("\x02TL"), QByteArray("\x02TS"));
      demo.replace(QByteArray("\x02TN"), QByteArray("\x02TS"));
      filename = cfgdir.absolutePath() + QStringLiteral("/Saves/") +
                 recordFileName + QStringLiteral(".") + cProtoVer +
                 QStringLiteral(".hws");
    }
    ...
    if (!demofile.open(QIODevice::WriteOnly))
      MessageDialog::ShowErrorMessage(
          tr("Cannot save record to file %1").arg(filename), this);
    else {
      demofile.write(demo);
      demofile.close();
    }
...
void HWForm::StartCampaign() {
  CreateGame(0, 0, 0);
  QString camp = ui.pageCampaign->CBCampaign
                     ->itemData(ui.pageCampaign->CBCampaign->currentIndex())
                     .toString();
  QString miss =
      campaignMissionInfo.at(ui.pageCampaign->CBMission->currentIndex()).script;
...
void HWForm::CreateNetGame() {
  ...
  QString ammo;
  ammo = ui.pageNetGame->pGameCFG->WeaponsName
             ->itemData(ui.pageNetGame->pGameCFG->WeaponsName->currentIndex())
             .toString();
...
void HWForm::NetGameMaster() {
  ...
  if (hwnet) {
    // disconnect connections first to ensure their inexistance and not to
    // connect twice
    ...
    connect(ui.pageNetGame->BtnStart, &QAbstractButton::clicked, this,
            &HWForm::startGame);
    connect(ui.pageNetGame, &PageNetGame::askForUpdateRoomName, hwnet.data(),
            &HWNewNet::updateRoomName);
    connect(ui.pageNetGame->restrictJoins, &QAction::triggered, hwnet.data(),
            &HWNewNet::toggleRestrictJoins);
    connect(ui.pageNetGame->restrictTeamAdds, &QAction::triggered, hwnet.data(),
            &HWNewNet::toggleRestrictTeamAdds);
...
void HWForm::NetLeftRoom(const QString &reason) {
  if (ui.Pages->currentIndex() == ID_PAGE_NETGAME ||
      ui.Pages->currentIndex() == ID_PAGE_INGAME) {
    GoBack();
    if (!reason.isEmpty()) ui.pageRoomsList->displayNotice(reason);
  } else
...
void HWForm::UpdateTrainingPageTeam(int index) {
  ...
  for (int w = 0; w < 3; w++) {
    switch (w) {
      case 0:
        listWidget = ui.pageTraining->lstTrainings;
        break;
      case 1:
        listWidget = ui.pageTraining->lstChallenges;
        break;
      case 2:
        listWidget = ui.pageTraining->lstScenarios;
        break;
      default:
        listWidget = ui.pageTraining->lstTrainings;
        break;
    ...
    for (unsigned int i = 0; i < n; i++) {
      ...
      QString missionName =
          QString(item->data(Qt::UserRole).toString())
...
void HWForm::InitCampaignPage() {
  ...
  QStringList entries = DataManager::instance().entryList(
      QStringLiteral("Missions/Campaign"), QDir::Dirs, QStringList("[^\\.]*"));
  ...
  for (unsigned int i = 0; i < n; i++) {
    ...
    QString tName = team.name();
    ui.pageCampaign->CBCampaign->addItem(getRealCampName(campaignName),
                                         campaignName);
  }
}
...
void HWForm::RestoreSingleplayerTeamSelection() {
  QString lastTeam =
      config->value("frontend/lastSingleplayerTeam", QString()).toString();
...
void HWForm::UpdateCampaignPage(int index) {
  ...
  for (int i = 0; i < campaignMissionInfo.size(); i++) {
    ui.pageCampaign->CBMission->addItem(
        QString(campaignMissionInfo[i].realName),
        QString(campaignMissionInfo[i].name));
...
void HWForm::UpdateCampaignPageTeam(int index) {
  ...
  for (unsigned int i = 0; i < n; i++) {
    QString campaignName =
        QString(entries[i]).replace(QStringLiteral(" "), QStringLiteral("_"));
...
void HWForm::UpdateCampaignPageMission(int index) {
  ...
  QString campaignName = ui.pageCampaign->CBCampaign->currentData().toString();
  // when campaign changes the UpdateCampaignPageMission is triggered with wrong
  // values this will cause segfault. This check prevents illegal memory reads
  if (index > -1 && index < campaignMissionInfo.count()) {
    ui.pageCampaign->lbltitle->setText(
        QStringLiteral("<h2>") + ui.pageCampaign->CBMission->currentText() +
        QStringLiteral("</h2>"));
    ui.pageCampaign->lbldescription->setText(
        campaignMissionInfo[index].description);
    ui.pageCampaign->btnPreview->setIcon(
        QIcon(campaignMissionInfo[index].image));
  }
}
...
void HWForm::UpdateCampaignPageProgress(int index) {
  ...
  if (index == gsFinished) {
    ...
    if (ui.pageCampaign->currentMissionWon == false &&
      ...
      if (!selected) {
        for (int m = ui.pageCampaign->CBMission->count() - 1;
             m > missionIndex - 1; m--) {
          if (!isCampMissionWon(campaignName, m, tName)) {
            missionIndex = m;
            break;
    ...
    ui.pageCampaign->currentMissionWon =
        isCampMissionWon(campaignName, missionIndex, tName);
  }
...
QString HWForm::getDemoArguments() {
  QString prefix =
      QStringLiteral("\"") + datadir.absolutePath() + QStringLiteral("\"");
  QString userPrefix =
      QStringLiteral("\"") + cfgdir.absolutePath() + QStringLiteral("\"");
#ifdef Q_OS_WIN
  ...
  return QString(QStringLiteral("--prefix ") + prefix +
                 ...
                 QStringLiteral(" --fullscreen-width ") +
                 QString::number(resolutions.first.width()) +
                 QStringLiteral(" --fullscreen-height ") +
                 QString::number(resolutions.first.height()) +
                 QStringLiteral(" --width ") +
                 QString::number(resolutions.second.width()) +
                 QStringLiteral(" --height ") +
                 QString::number(resolutions.second.height()) +
                 (config->vid_Maximized() ? " --maximized" : "") +
                 (config->zoom() == 100 ? QLatin1String("")
                                        : QStringLiteral(" --zoom ") +
                                              QString::number(config->zoom())) +
                 QStringLiteral(" --volume ") +
                 QString::number(config->volume()) +
                 (config->isMusicEnabled() ? "" : " --nomusic") +
                 (config->isSoundEnabled() ? "" : " --nosound") +
                 (config->isAudioDampenEnabled() ? "" : " --nodampen") +
                 QStringLiteral(" --locale ") + config->language() +
                 QStringLiteral(".txt") +
                 ...
                 QStringLiteral(" --raw-quality ") +
                 QString::number(config->translateQuality())) +
         (!config->Form->ui.pageOptions->CBTeamTag->isChecked()
              ? " --no-teamtag"
              : "") +
         (!config->Form->ui.pageOptions->CBHogTag->isChecked() ? " --no-hogtag"
                                                               : "") +
         (!config->Form->ui.pageOptions->CBHealthTag->isChecked()
              ? " --no-healthtag"
              : "") +
         (config->Form->ui.pageOptions->CBTagOpacity->isChecked()
              ? " --translucent-tags"
              : "") +
...
void HWForm::AssociateFiles() {
  ...
  registry_hkcr.setValue("Hedgewars.Demo/Default",
                         tr("Hedgewars Demo File", "File Types"));
  registry_hkcr.setValue("Hedgewars.Save/Default",
  ...
  // custom url scheme(s)
  registry_hkcr.setValue("hwplay/Default",
                         "\"URL:Hedgewars ServerAccess Scheme\"");
  ...
  // and you want to reset it
  system(
      "defaults write com.apple.LaunchServices LSHandlers -array-add "
      "'<dict><key>LSHandlerContentTag</key><string>hwd</"
      "string><key>LSHandlerContentTagClass</"
      "key><string>public.filename-extension</string><key>LSHandlerRoleAll</"
      "key><string>org.hedgewars.desktop</string></dict>'");
  system(
      "defaults write com.apple.LaunchServices LSHandlers -array-add "
      "'<dict><key>LSHandlerContentTag</key><string>hws</"
      "string><key>LSHandlerContentTagClass</"
      "key><string>public.filename-extension</string><key>LSHandlerRoleAll</"
      "key><string>org.hedgewars.desktop</string></dict>'");
  system(
      "/System/Library/Frameworks/CoreServices.framework/Versions/A/Frameworks/"
      "LaunchServices.framework/Versions/A/Support/lsregister -kill -domain "
      "local -domain system -domain user");
#else
  // this is a little silly due to all the system commands below anyway - just
  // use mkdir -p ?  Does have the advantage of the alert I guess
  if (success)
    success =
        checkForDir(QDir::home().absolutePath() + QStringLiteral("/.local"));
  if (success)
    success = checkForDir(QDir::home().absolutePath() +
  ...
  if (success)
    success = checkForDir(QDir::home().absolutePath() +
                          QStringLiteral("/.local/share/mime/packages"));
  if (success)
    success =
        checkForDir(QDir::home().absolutePath() + QStringLiteral("/.local"));
  if (success)
  ...
  if (success)
    success = QFile::copy(
        ...
        QDir::home().absolutePath() +
            QStringLiteral("/.local/share/applications"));
  if (success)
    success = system((QStringLiteral("update-mime-database ") +
                      QDir::home().absolutePath() +
                      QStringLiteral("/.local/share/mime"))
                         .toLocal8Bit()
                         .constData()) == 0;
  if (success)
    success =
        system("xdg-mime default hedgewars.desktop x-scheme-handler/hwplay") ==
        0;
  if (success)
    success =
        system(
            "xdg-mime default hwengine.desktop application/x-hedgewars-demo") ==
        0;
  if (success)
    success =
        system(
            "xdg-mime default hwengine.desktop application/x-hedgewars-save") ==
        0;
  // hack to add user's settings to hwengine. might be better at this point to
  // read in the file, append it, and write it out to its new home.  This
  // assumes no spaces in the data dir path
  if (success)
    success =
        system(("sed -i 's|^\\(Exec=.*\\) \\(%f\\)|\\1 \\2 " + arguments +
                QStringLiteral("|' ") + QDir::home().absolutePath() +
                QStringLiteral("/.local/share/applications/hwengine.desktop"))
                   .toLocal8Bit()
                   .constData()) == 0;
#endif
  if (success) {
    ...
    MessageDialog::ShowErrorMessage(QMessageBox::tr("File association failed."),
                                    this);
}

void HWForm::openRegistrationPage() {
  QDesktopServices::openUrl(
      QUrl(QStringLiteral("https://www.hedgewars.org/user/register")));
}
...
void HWForm::saveDemoWithCustomName() {
  if (!m_lastDemo.isEmpty()) {
    ...
    do {
      ...
      if (!fileName.isEmpty()) {
        QString filePath = cfgdir.absolutePath() + QStringLiteral("/Demos/") +
                           fileName + QStringLiteral(".") + cProtoVer +
                           QStringLiteral(".hwd");
        QFile demofile(filePath);
        ok = demofile.open(QIODevice::WriteOnly);
        if (!ok)
          MessageDialog::ShowErrorMessage(
              tr("Cannot save record to file %1").arg(filePath), this);
        else {
...
void HWForm::restartGame() {
  ...
  if (ui.Pages->currentIndex() == ID_PAGE_GAMESTATS) GoBack();
  ...
  switch (lastGameType) {
    case gtTraining:
      game->StartTraining(lastGameStartArgs.at(0).toString(),
                          lastGameStartArgs.at(1).toString(),
                          lastGameStartArgs.at(2).toString());
      break;
    case gtQLocal:
      game->StartQuick();
      break;
    case gtCampaign:
      game->StartCampaign(lastGameStartArgs.at(0).toString(),
                          lastGameStartArgs.at(1).toString(),
                          lastGameStartArgs.at(2).toString());
      break;
    case gtLocal:
...
void HWForm::showFeedbackDialogNetChecked() {
  ...
  if (reply) {
    switch (reply->error()) {
      ...
      default:
        MessageDialog::ShowFatalMessage(
            QString(tr("This feature requires an Internet connection, but you "
                       "don't appear to be online (error code: %1)."))
                .arg(reply->error()),
            this);
        break;
    }
    ...
    MessageDialog::ShowFatalMessage(
        tr("Internal error: Reply object is invalid."), this);
  }
}
...
void HWForm::startGame() {
  ...
  questionMsg.setText(QMessageBox::tr(
