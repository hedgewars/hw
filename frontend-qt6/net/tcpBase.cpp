...
EngineInstance::EngineInstance(QObject *parent) : QObject(parent) {}
...
TCPBase::~TCPBase() {
  ...
  if (IPCSocket) IPCSocket->deleteLater();
}
  ...
  if (!IPCServer) {
    ...
    if (!IPCServer->listen(QHostAddress::LocalHost)) {
      MessageDialog::ShowFatalMessage(
          tr("Unable to start server at %1.").arg(IPCServer->errorString()));
      exit(0);  // FIXME - should be graceful exit here (lower Critical ->
                // Warning above when implemented)
    }
  }
...
void TCPBase::NewConnection() {
  ...
  disconnect(IPCServer.data(), &QTcpServer::newConnection, this,
  ...
  connect(IPCSocket.data(), &QAbstractSocket::disconnected, this,
          &TCPBase::ClientDisconnect);
...
void TCPBase::ClientDisconnect() {
  ...
  if (IPCSocket) {
    disconnect(IPCSocket.data(), &QIODevice::readyRead, this,
               &TCPBase::ClientRead);
...
void TCPBase::onEngineDeath(int exitCode, QProcess::ExitStatus exitStatus) {
  ...
  if (!m_connected) {    // yes, it is intended to be like this
    ...
    // if m_connected is true, it is done automatically in socket disconnect
    // handler
  }
  ...
  if ((exitCode != HWENGINE_EXITCODE_OK) &&
      (exitCode != HWENGINE_EXITCODE_FATAL)) {
    // inform user that something bad happened
    MessageDialog::ShowFatalMessage(
        tr("The game engine died unexpectedly!\n"
           "(exit code %1)\n\n"
           "We are very sorry for the inconvenience :(\n\n"
           "If this keeps happening, please click the '%2' button in the main "
           "menu!")
            .arg(exitCode)
...
void TCPBase::tcpServerReady() {
  if (!srvsList.isEmpty()) {
    disconnect(srvsList.first(), &TCPBase::isReadyNow, this,
               &TCPBase::tcpServerReady);
    RealStart();
  } else {
    qDebug(
        "tcpServerReady() called while srvsList was empty. Not starting TCP "
        "server");
  }
}
...
void TCPBase::Start(bool couldCancelPreviousRequest) {
  if (srvsList.isEmpty()) {
    ...
    if (couldCancelPreviousRequest && last->couldBeRemoved() &&
...
bool TCPBase::isConnected() { return m_connected; }

bool TCPBase::simultaneousRun() { return false; }

bool TCPBase::hasStarted() { return m_hasStarted; }
