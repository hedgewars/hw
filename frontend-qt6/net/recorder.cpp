...
HWRecorder::HWRecorder(GameUIConfig *config, const QString &prefix)
  ...
  finished = false;
  aborted = false;
  name = prefix + QStringLiteral(".") +
         LibavInteraction::instance().getExtension(config->AVFormat());
}
  ...
  while (!readbuffer.isEmpty() && ((bufsize = readbuffer.size()) > 0) &&
    ...
    switch (msg.at(1)) {
      case '?':
        ...
        break;
      case 'p':
        Q_EMIT onProgress((quint8(msg.at(2)) * 256.0 + quint8(msg.at(3))) *
                          0.0001);
        break;
      case 'v':
        finished = true;
      ...
      case 'E':
        ...
        Q_EMIT ErrorMessage(
            tr("A fatal ERROR occured while processing the video recording! "
               ...
               "As a workaround, you could try to reset the Hedgewars video "
               "recorder settings to the defaults.\n\n"
               "To report this error, please click the 'Feedback' button in "
               "the main menu!\n\n"
               "Last engine message:\n%1")
...
QStringList HWRecorder::getArguments() {
  ...
  // Could use a field to use quality instead. maybe quality could override
  // bitrate - or just pass (and set) both. The library does support using both
  // at once after all.
  arguments << QString::number(config->rec_Bitrate() * 1024);
  if (config->recordAudio() &&
      (config->isSoundEnabled() || config->isMusicEnabled()))
...
bool HWRecorder::simultaneousRun() { return true; }
