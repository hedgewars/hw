...
HWTeam::HWTeam(const QStringList& strLst)
    : QObject(0), m_numHedgehogs(4), m_isNetTeam(true), m_isMissionTeam(false) {
  ...
  for (int i = 0; i < HEDGEHOGS_PER_TEAM; i++) {
    ...
    // Somehow claymore managed an empty hat.  Until we figure out how, this
    // should avoid a repeat Checking net teams is probably pointless, but can't
    // hurt.
...
bool HWTeam::loadFromFile() {
  ...
  for (int i = 0; i < HEDGEHOGS_PER_TEAM; i++) {
    ...
    m_hedgehogs[i].Name = teamfile
                              .value(hh + QStringLiteral("Name"),
                                     QStringLiteral("Hedgehog %1").arg(i + 1))
                              .toString();
    m_hedgehogs[i].Hat =
        teamfile.value(hh + QStringLiteral("Hat"), "NoHat").toString();
  }
  for (int i = 0; i < BINDS_NUMBER; i++)
    m_binds[i].strbind =
        teamfile
            .value(QStringLiteral("Binds/%1").arg(m_binds[i].action), QString())
            .toString();
  for (int i = 0; i < MAX_ACHIEVEMENTS; i++)
    if (achievements[i][0][0])
      AchievementProgress[i] =
          teamfile
              .value(QStringLiteral("Achievements/%1").arg(achievements[i][0]),
                     0)
              .toUInt();
    else
      break;
...
// Returns true if the team name has been changed but a file with the same team
// name already exists. So if this team would be saved, another team file would
// be overwritten, which is generally not desired.
bool HWTeam::wouldOverwriteOtherFile() {
  return (m_name != OldTeamName) && fileExists();
}

bool HWTeam::deleteFile() {
  if (m_isNetTeam) return false;
...
bool HWTeam::saveToFile() {
  ...
  for (int i = 0; i < BINDS_NUMBER; i++) {
    if (QString(m_binds[i].action) != QStringLiteral("!MULTI"))
      teamfile.setValue(QStringLiteral("Binds/%1").arg(m_binds.at(i).action),
                        m_binds[i].strbind);
  }
  for (int i = 0; i < MAX_ACHIEVEMENTS; i++)
    if (achievements[i][0][0])
      teamfile.setValue(
          QStringLiteral("Achievements/%1").arg(achievements[i][0]),
          AchievementProgress[i]);
    else
      break;
...
QStringList HWTeam::teamGameConfig(quint32 InitHealth) const {
  ...
  if (m_isNetTeam) {
    sl.push_back(
        QStringLiteral("eaddteam %3 %1 %2")
            .arg(qcolor().rgb() & 0xffffff)
            .arg(m_name, QString(QCryptographicHash::hash(
                                     m_owner.toUtf8(), QCryptographicHash::Md5)
                                     .toHex())));
    sl.push_back(QStringLiteral("erdriven"));
  } else {
    if (m_isMissionTeam) {
      sl.push_back(QStringLiteral("esetmissteam %3 %1 %2")
                       .arg(qcolor().rgb() & 0xffffff)
                       .arg(m_name, playerHash));
      cmdAddHog = QStringLiteral("eaddmisshh");
    } else {
      sl.push_back(QStringLiteral("eaddteam %3 %1 %2")
                       .arg(qcolor().rgb() & 0xffffff)
                       .arg(m_name, playerHash));
    }
  }
  ...
  if (!m_owner.isEmpty()) sl.push_back(QStringLiteral("eowner ") + m_owner);

  for (int t = 0; t < m_numHedgehogs; t++) {
    sl.push_back(QString(cmdAddHog + QStringLiteral(" %1 %2 %3"))
                     .arg(QString::number(m_difficulty),
                          QString::number(InitHealth), m_hedgehogs[t].Name));
    sl.push_back(QStringLiteral("ehat %1").arg(m_hedgehogs[t].Hat));
  }
  return sl;
}

void HWTeam::setNetTeam(bool isNetTeam) { m_isNetTeam = isNetTeam; }

bool HWTeam::isNetTeam() const { return m_isNetTeam; }

void HWTeam::setMissionTeam(bool isMissionTeam) {
  m_isMissionTeam = isMissionTeam;
}

bool HWTeam::isMissionTeam() const { return m_isMissionTeam; }

bool HWTeam::operator==(const HWTeam& t1) const { return m_name == t1.m_name; }
...
void HWTeam::setOwner(const QString& owner) { m_owner = owner; }
...
// color
int HWTeam::color() const { return m_color; }
