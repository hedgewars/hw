...
#else
  if (ofmt->codec_tag) return !!av_codec_get_tag(ofmt->codec_tag, codec_id);
...
#endif
  {
    if (!av_codec_is_encoder(pCodec)) continue;

    if (pCodec->type != AVMEDIA_TYPE_VIDEO &&
        pCodec->type != AVMEDIA_TYPE_AUDIO)
      continue;
    ...
    if (strcmp(pCodec->name, "libaom-av1") == 0) continue;

    // doesn't support stereo sound
    if (strcmp(pCodec->name, "real_144") == 0) continue;

    if (!pCodec->long_name || strlen(pCodec->long_name) == 0) continue;

    if (pCodec->type == AVMEDIA_TYPE_VIDEO) {
      if (pCodec->supported_framerates != NULL) continue;

      // check if codec supports yuv 4:2:0 format
      if (!pCodec->pix_fmts) continue;
      bool yuv420Supported = false;
      for (const enum AVPixelFormat* pfmt = pCodec->pix_fmts; *pfmt != -1;
           pfmt++)
        if (*pfmt == AV_PIX_FMT_YUV420P) {
          yuv420Supported = true;
          break;
        }
      if (!yuv420Supported) continue;
    }
    if (pCodec->type == AVMEDIA_TYPE_AUDIO) {
      // check if codec supports signed 16-bit format
      if (!pCodec->sample_fmts) continue;
      bool s16Supported = false;
      for (const enum AVSampleFormat* pfmt = pCodec->sample_fmts; *pfmt != -1;
           pfmt++)
        if (*pfmt == AV_SAMPLE_FMT_S16) {
          s16Supported = true;
          break;
        }
      if (!s16Supported) continue;
    }
    ...
    if (strcmp(pCodec->name, "mpeg4") == 0 ||
        strcmp(pCodec->name, "ac3_fixed") == 0)
      codec.isRecommended = true;
  }
...
#endif
  {
    if (!pFormat->extensions) continue;

    // skip some strange formats to not confuse users
    if (strstr(pFormat->long_name, "raw")) continue;

    Format format;
    bool hasVideoCodec = false;
    for (QList<Codec>::iterator codec = codecs.begin(); codec != codecs.end();
         ++codec) {
      if (!FormatQueryCodec(pFormat, codec->id)) continue;
      format.codecs.push_back(&*codec);
      if (!codec->isAudio) hasVideoCodec = true;
    }
    if (!hasVideoCodec) continue;
    ...
    format.isRecommended =
        strcmp(pFormat->name, "mp4") == 0 || strcmp(pFormat->name, "avi") == 0;
...
void LibavInteraction::fillFormats(QComboBox* pFormats) {
  ...
  if (sep != 0 && sep != pFormats->count()) pFormats->insertSeparator(sep);
}

void LibavInteraction::fillCodecs(const QString& fmt, QComboBox* pVCodecs,
                                  QComboBox* pACodecs) {
  ...
  if (vsep != 0 && vsep != pVCodecs->count()) pVCodecs->insertSeparator(vsep);
  if (asep != 0 && asep != pACodecs->count()) pACodecs->insertSeparator(asep);
}
...
QString LibavInteraction::getFileInfo(const QString& filepath) {
  ...
  if (avformat_open_input(&pContext, utf8path.data(), NULL, NULL) < 0)
    return "";
  if (avformat_find_stream_info(pContext, NULL) < 0) return "";
  ...
  for (int i = 0; i < (int)pContext->nb_streams; i++) {
    ...
    if (!pStream) continue;
#if LIBAVCODEC_VERSION_MAJOR >= 59
    const AVCodec* st_codec =
        avcodec_find_decoder(pContext->streams[i]->codecpar->codec_id);
    ...
    if (pCodec->codec_type == AVMEDIA_TYPE_VIDEO) {
      if (pStream->avg_frame_rate.den) {
        float fps =
            float(pStream->avg_frame_rate.num) / pStream->avg_frame_rate.den;
        //: Video metadata. %1 = video width, %2 = video height, %3 = frames per
        //: second = %4 = decoder name
        desc += QString(tr("Video: %1x%2, %3 FPS, %4"))
                    .arg(pCodec->width)
                    .arg(pCodec->height)
                    .arg(QLocale().toString(fps, 'f', 2))
                    .arg(decoderName);
      } else {
        //: Video metadata. %1 = video width, %2 = video height, %3 = decoder
        //: name
        desc += QString(tr("Video: %1x%2, %3"))
                    .arg(pCodec->width)
                    .arg(pCodec->height)
                    .arg(decoderName);
      }
  ...
  AVDictionaryEntry* pComment =
      av_dict_get(pContext->metadata, "comment", NULL, 0);
  if (pComment) {
    // Video comment. We expect a simple key value storage in a particular
    // format and parse it here so the key names can be localized.
    desc += QString("\n");
...
void LibavInteraction::fillFormats(QComboBox* pFormats) { Q_UNUSED(pFormats); }

void LibavInteraction::fillCodecs(const QString& format, QComboBox* pVCodecs,
                                  QComboBox* pACodecs) {
  Q_UNUSED(format);
  Q_UNUSED(pVCodecs);
  Q_UNUSED(pACodecs);
