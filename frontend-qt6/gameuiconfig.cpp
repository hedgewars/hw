...
const QNetworkProxy::ProxyType proxyTypesMap[] = {
    QNetworkProxy::NoProxy, QNetworkProxy::NoProxy  // dummy value
    ,
...
GameUIConfig::GameUIConfig(HWForm *FormWidgets, const QString &fileName)
  ...
  // Form->resize(value("frontend/width", 640).toUInt(),
  // value("frontend/height", 450).toUInt());
...
void GameUIConfig::reloadValues(void) {
  Form->ui.pageOptions->WeaponTooltip->setChecked(
      value("misc/weaponTooltips", true).toBool());

  int t = Form->ui.pageOptions->CBResolution->findText(
      value("video/fullscreenResolution").toString());
  if (t < 0) {
    ...
    Form->ui.pageOptions->CBResolution->setCurrentIndex(t);
  ...
  Form->ui.pageOptions->CBFullscreen->setChecked(
      value("video/fullscreen", false).toBool());
  ...
  Form->ui.pageOptions->CBStereoMode->setCurrentIndex(
      value("video/stereo", 0).toUInt());
  Form->ui.pageOptions->CBFrontendEffects->setChecked(
      value("frontend/effects", true).toBool());
  Form->ui.pageOptions->CBSound->setChecked(
      value("audio/sound", true).toBool());
  Form->ui.pageOptions->CBFrontendSound->setChecked(
      value("frontend/sound", true).toBool());
  Form->ui.pageOptions->CBMusic->setChecked(
      value("audio/music", true).toBool());
  Form->ui.pageOptions->CBFrontendMusic->setChecked(
      value("frontend/music", true).toBool());
  Form->ui.pageOptions->CBDampenAudio->setChecked(
      value("audio/dampen", true).toBool());
  ...
  netHost = QString(value("net/ip", "").toString());
  netPort = value("net/port", NETGAME_DEFAULT_PORT).toUInt();

  Form->ui.pageNetServer->leServerDescr->setText(
      value("net/servername", "Hedgewars Server").toString());
  Form->ui.pageNetServer->sbPort->setValue(
      value("net/serverport", NETGAME_DEFAULT_PORT).toUInt());

  Form->ui.pageOptions->CBShowFPS->setChecked(
      value("fps/show", false).toBool());
  Form->ui.pageOptions->fpsedit->setValue(value("fps/limit", 27).toUInt());

  Form->ui.pageOptions->CBAltDamage->setChecked(
      value("misc/altdamage", true).toBool());
  Form->ui.pageOptions->CBNameWithDate->setChecked(
      value("misc/appendTimeToRecords", false).toBool());

  Form->ui.pageOptions->CBTeamTag->setChecked(
      value("misc/teamtag", true).toBool());
  Form->ui.pageOptions->CBHogTag->setChecked(
      value("misc/hogtag", true).toBool());
  Form->ui.pageOptions->CBHealthTag->setChecked(
      value("misc/healthtag", true).toBool());
  Form->ui.pageOptions->CBTagOpacity->setChecked(
      value("misc/tagopacity", false).toBool());

#ifdef SPARKLE_ENABLED
  Form->ui.pageOptions->CBAutoUpdate->setChecked(
      value("misc/autoUpdate", true).toBool());
#endif

  Form->ui.pageOptions->CBLanguage->setCurrentIndex(
      Form->ui.pageOptions->CBLanguage->findData(
          value("misc/locale", "").toString()));

  Form->ui.pageOptions->cbProxyType->setCurrentIndex(
      value("proxy/type", 0).toInt());
  Form->ui.pageOptions->leProxy->setText(value("proxy/host", "").toString());
  Form->ui.pageOptions->sbProxyPort->setValue(
      value("proxy/port", "8080").toInt());
  Form->ui.pageOptions->leProxyLogin->setText(
      value("proxy/login", "").toString());
  Form->ui.pageOptions->leProxyPassword->setText(
      value("proxy/password", "").toString());
  ...
  {  // load colors
    ...
    for (int i = model->rowCount() - 1; i >= 0; --i)
      model->item(i)->setData(QColor(
          value(QStringLiteral("colors/color%1").arg(i), model->item(i)->data())
              .toString()));
  }
  ...
  {  // load binds
    for (int i = 0; i < BINDS_NUMBER; i++) {
      m_binds[i].strbind =
          value(QStringLiteral("Binds/%1").arg(m_binds[i].action),
                cbinds[i].strbind)
              .toString();
...
void GameUIConfig::reloadVideosValues(void) {
  ...
  int framerateBoxIndex = Form->ui.pageOptions->framerateBox->findData(
      value("videorec/framerate", rec_Framerate()).toUInt());
  if (framerateBoxIndex != -1)
    Form->ui.pageOptions->framerateBox->setCurrentIndex(framerateBoxIndex);
  Form->ui.pageOptions->bitrateBox->setValue(
      value("videorec/bitrate", rec_Bitrate()).toUInt());
  bool useGameRes = value("videorec/usegameres",
                          Form->ui.pageOptions->checkUseGameRes->isChecked())
                        .toBool();
  if (useGameRes) {
    ...
    Form->ui.pageOptions->widthEdit->setText(
        value("videorec/width", "800").toString());
    Form->ui.pageOptions->heightEdit->setText(
        value("videorec/height", "600").toString());
  }
  Form->ui.pageOptions->checkUseGameRes->setChecked(useGameRes);
  Form->ui.pageOptions->checkRecordAudio->setChecked(
      value("videorec/audio",
            Form->ui.pageOptions->checkRecordAudio->isChecked())
          .toBool());
  if (!Form->ui.pageOptions->tryCodecs(
          value("videorec/format", "no").toString(),
...
QStringList GameUIConfig::GetTeamsList() {
  ...
  QStringList teamslist =
      teamdir.entryList(QStringList("*.hwt"), QDir::Files | QDir::Hidden);
  QStringList cleanedList;
  for (QStringList::Iterator it = teamslist.begin(); it != teamslist.end();
       ++it) {
    QString tmpTeamStr = (*it).replace(
        QRegularExpression(QStringLiteral("^(.*)\\.hwt$")), "\\1");
...
void GameUIConfig::resizeToConfigValues() {
  ...
  const QRect deskSize = qGuiApp->primaryScreen()->geometry();
  Form->resize(
      value("frontend/width",
            qMin(qMax(deskSize.width() * 2 / 3, 800), deskSize.width()))
          .toUInt(),
      value("frontend/height",
            qMin(qMax(deskSize.height() * 2 / 3, 600), deskSize.height()))
          .toUInt());
  ...
  QPoint center = qGuiApp->primaryScreen()->availableGeometry().center();
...
void GameUIConfig::SaveOptions() {
  setValue(QStringLiteral("video/fullscreenResolution"),
           Form->ui.pageOptions->CBResolution->currentText());
  setValue(QStringLiteral("video/windowedWidth"),
           Form->ui.pageOptions->windowWidthEdit->value());
  setValue(QStringLiteral("video/windowedHeight"),
           Form->ui.pageOptions->windowHeightEdit->value());
  setValue(QStringLiteral("video/windowedMaximized"), vid_Maximized());
  setValue(QStringLiteral("video/fullscreen"), vid_Fullscreen());

  setValue(QStringLiteral("video/quality"),
           Form->ui.pageOptions->SLQuality->value());
  ...
  setValue(QStringLiteral("misc/weaponTooltips"),
           Form->ui.pageOptions->WeaponTooltip->isChecked());
  ...
  if (!ffscr) {
    ...
    setValue(QStringLiteral("frontend/maximized"),
             (Form->windowState() & Qt::WindowMaximized) != 0);
  } else {
  ...
  setValue(QStringLiteral("audio/volume"),
           Form->ui.pageOptions->SLVolume->value());
  setValue(QStringLiteral("audio/dampen"), isAudioDampenEnabled());

  setValue(QStringLiteral("net/nick"), netNick());
  if (netPasswordIsValid() &&
      Form->ui.pageOptions->CBSavePassword->isChecked()) {
    setPasswordHash(netPasswordHash());
  } else if (!Form->ui.pageOptions->CBSavePassword->isChecked()) {
    clearPasswordHash();
  }

  setValue(QStringLiteral("net/savepassword"),
           Form->ui.pageOptions->CBSavePassword->isChecked());
  setValue(QStringLiteral("net/ip"), netHost);
  setValue(QStringLiteral("net/port"), netPort);
  setValue(QStringLiteral("net/servername"),
           Form->ui.pageNetServer->leServerDescr->text());
  setValue(QStringLiteral("net/serverport"),
           Form->ui.pageNetServer->sbPort->value());
  ...
  setValue(QStringLiteral("misc/teamtag"),
           Form->ui.pageOptions->CBTeamTag->isChecked());
  setValue(QStringLiteral("misc/hogtag"),
           Form->ui.pageOptions->CBHogTag->isChecked());
  setValue(QStringLiteral("misc/healthtag"),
           Form->ui.pageOptions->CBHealthTag->isChecked());
  setValue(QStringLiteral("misc/tagopacity"),
           Form->ui.pageOptions->CBTagOpacity->isChecked());

  setValue(QStringLiteral("misc/appendTimeToRecords"),
           appendDateTimeToRecordName());
  ...
  {  // setup proxy
    ...
    if (proxyType == PageOptions::Socks5Proxy ||
        proxyType == PageOptions::HTTPProxy) {
      setValue(QStringLiteral("proxy/host"),
               Form->ui.pageOptions->leProxy->text());
      setValue(QStringLiteral("proxy/port"),
               Form->ui.pageOptions->sbProxyPort->value());
      setValue(QStringLiteral("proxy/login"),
               Form->ui.pageOptions->leProxyLogin->text());
      setValue(QStringLiteral("proxy/password"),
               Form->ui.pageOptions->leProxyPassword->text());
    }
  ...
  {  // save colors
    QStandardItemModel *model = DataManager::instance().colorsModel();
    for (int i = model->rowCount() - 1; i >= 0; --i)
      setValue(QStringLiteral("colors/color%1").arg(i),
               model->item(i)->data().value<QColor>().name());
  }

  setValue(QStringLiteral("chat/size"),
           Form->ui.pageOptions->sbChatSize->value());
...
void GameUIConfig::SaveVideosOptions() {
  ...
  setValue(QStringLiteral("videorec/usegameres"),
           Form->ui.pageOptions->checkUseGameRes->isChecked());
...
QString GameUIConfig::language() {
  return Form->ui.pageOptions->CBLanguage
      ->itemData(Form->ui.pageOptions->CBLanguage->currentIndex())
      .toString();
}
...
bool GameUIConfig::vid_Maximized() { return pIsEngineWindowMaximized; }
...
quint32 GameUIConfig::translateQuality() {
  ...
  quint32 result = (Form->ui.pageOptions->WeaponTooltip->isChecked())
                       ? rqNone
                       : rqTooltipsOff;
...
quint16 GameUIConfig::zoom() { return Form->ui.pageOptions->SLZoom->value(); }
...
QString GameUIConfig::getRandomNick() {
  ...
  if (cachedRandomNick.isNull())
    ...
    cachedRandomNick =
        tr("Guest") + QStringLiteral("%1").arg(rand() % 99999 + 1);
  return cachedRandomNick;
}

QByteArray GameUIConfig::netPasswordHash() {
  return QCryptographicHash::hash(
             Form->ui.pageOptions->editNetPassword->text().toUtf8(),
...
bool GameUIConfig::netPasswordIsValid() {
  return (netPasswordLength() == 0 ||
          Form->ui.pageOptions->editNetPassword->text() !=
              QString(netPasswordLength(), '*'));
}
...
void GameUIConfig::clearPasswordHash() {
  ...
  setValue(QStringLiteral("net/savepassword"),
           false);  // changes the savepassword value to false in order to not
                    // let the user save an empty password in PAGE_SETUP
  Form->ui.pageOptions->editNetPassword->setEnabled(false);
  Form->ui.pageOptions->editNetPassword->setText(QLatin1String(""));
}
...
void GameUIConfig::setPasswordHash(const QString &passwordhash) {
  ...
  if (!passwordhash.isEmpty()) {
    ...
    // divided by 4 let PAGE_SETUP use a reasonable number of stars to display
    // the PW
...
void GameUIConfig::clearTempHash() { setTempHash(QString()); }

void GameUIConfig::setTempHash(const QString &temphash) {
  this->temphash = temphash;
}

QString GameUIConfig::tempHash() { return this->temphash; }

// When hedgewars launches, the password field is set with null characters. If
// the user tries to edit the field and there are such characters, then clear
// the field
...
QString GameUIConfig::AVFormat() { return Form->ui.pageOptions->format(); }
...
int GameUIConfig::rec_Framerate() {
  return Form->ui.pageOptions->framerateBox
      ->itemData(Form->ui.pageOptions->framerateBox->currentIndex())
      .toInt();
}
...
void GameUIConfig::setBind(int bindID, QString &strbind) {
  ...
  setValue(QStringLiteral("Binds/%1").arg(m_binds.at(bindID).action), strbind);
}
