...
void PlayersListModel::setFlag(const QString &nickname, StateFlag flagType,
  ...
  if (flagType == Friend) {
    ...
    saveSet(m_friendsSet, QStringLiteral("friends"));
    ...
    saveSet(m_ignoredSet, QStringLiteral("ignore"));
  }
  ...
  if (m_icons().contains(iconNum)) {
    ...
    if (index.data(RoomFilterRole).toBool()) {
      if (index.data(InGame).toBool()) {
        painter.drawPixmap(0, 0, 16, 16,
                           QPixmap(QStringLiteral(":/res/chat/ingame.png")));
      } else {
        if (index.data(Ready).toBool())
          painter.drawPixmap(0, 0, 16, 16,
                             QPixmap(QStringLiteral(":/res/chat/lamp.png")));
        else
          painter.drawPixmap(
              0, 0, 16, 16, QPixmap(QStringLiteral(":/res/chat/lamp_off.png")));
      }
    } else {  // we're in lobby
      if (!index.data(InRoom).toBool())
        painter.drawPixmap(0, 0, 16, 16,
                           QPixmap(QStringLiteral(":/res/Flake.png")));
    }

    QString mainIconName(QStringLiteral(":/res/chat/"));

    if (index.data(ServerAdmin).toBool())
      mainIconName += QLatin1String("serveradmin");
    else {
      if (index.data(RoomAdmin).toBool())
        mainIconName += QLatin1String("roomadmin");
      else
        mainIconName += QLatin1String("hedgehog");

      if (index.data(Contributor).toBool())
        mainIconName += QLatin1String("contributor");
    }

    if (!index.data(Registered).toBool())
      mainIconName += QLatin1String("_gray");

    painter.drawPixmap(8, 0, 16, 16,
                       QPixmap(mainIconName + QStringLiteral(".png")));

    if (index.data(Ignore).toBool())
      painter.drawPixmap(8, 0, 16, 16,
                         QPixmap(QStringLiteral(":/res/chat/ignore.png")));
    else if (index.data(Friend).toBool())
      painter.drawPixmap(8, 0, 16, 16,
                         QPixmap(QStringLiteral(":/res/chat/friend.png")));
...
void PlayersListModel::setNickname(const QString &nickname) {
  ...
  loadSet(m_friendsSet, QStringLiteral("friends"));
  loadSet(m_ignoredSet, QStringLiteral("ignore"));
...
void PlayersListModel::loadSet(QSet<QString> &set, const QString &suffix) {
  ...
  QString fileName =
      QStringLiteral("%1/%2_%3.txt")
          .arg(cfgdir.absolutePath(), m_nickname.toLower(), suffix);
  ...
  while (!stream.atEnd()) {
    ...
    if (str.isEmpty() || str.front() == ';') continue;
...
void PlayersListModel::saveSet(const QSet<QString> &set,
  ...
  stream << "; this list is used by Hedgewars - do not edit it unless you "
            "know what you're doing!\n";

  for (auto nick : set.values()) {
    stream << nick << "\n";
  }
