...
GameSchemeModel::GameSchemeModel(QObject *parent, const QString &directory)
  ...
  predefSchemesNames =
      QStringList() << QStringLiteral("Default") << QStringLiteral("Pro Mode")
                    << QStringLiteral("Shoppa") << QStringLiteral("Clean Slate")
                    ...
                    << QStringLiteral("King Mode") << QStringLiteral("Mutant")
                    << QStringLiteral("Construction Mode")
                    << QStringLiteral("The Specialists")
                    << QStringLiteral("Space Invasion")
                    << QStringLiteral("HedgeEditor") << QStringLiteral("Racer");

  numberOfDefaultSchemes = predefSchemesNames.size();

  spNames = QStringList() << QStringLiteral(
  ...
  proMode << predefSchemesNames[1]  // name           0
          << QVariant(false)        // switchhog      1
          << QVariant(false)        // team divide    2
          << QVariant(false)        // solid land     3
          << QVariant(false)        // border         4
  ...
  shoppa << predefSchemesNames[2]  // name           0
         << QVariant(false)        // switchhog      1
         << QVariant(false)        // team divide    2
  ...
  cleanslate << predefSchemesNames[3]  // name           0
             << QVariant(false)        // switchhog      1
             << QVariant(false)        // team divide    2
             << QVariant(false)        // solid land     3
             << QVariant(false)        // border         4
  ...
  minefield << predefSchemesNames[4]  // name           0
            << QVariant(false)        // switchhog      1
            << QVariant(false)        // team divide    2
            << QVariant(false)        // solid land     3
            << QVariant(false)        // border         4
  ...
  barrelmayhem << predefSchemesNames[5]  // name           0
               << QVariant(false)        // switchhog      1
               << QVariant(false)        // team divide    2
               << QVariant(false)        // solid land     3
               << QVariant(false)        // border         4
  ...
  tunnelhogs << predefSchemesNames[6]  // name           0
             << QVariant(false)        // switchhog      1
             << QVariant(false)        // team divide    2
             << QVariant(false)        // solid land     3
             << QVariant(true)         // border         4
  ...
  timeless << predefSchemesNames[7]  // name           0
           << QVariant(false)        // switchhog      1
           << QVariant(false)        // team divide    2
           << QVariant(false)        // solid land     3
           << QVariant(false)        // border         4
  ...
  thinkingportals << predefSchemesNames[8]  // name           0
                  << QVariant(false)        // switchhog      1
                  << QVariant(false)        // team divide    2
                  << QVariant(false)        // solid land     3
                  << QVariant(false)        // border         4
  ...
  kingmode << predefSchemesNames[9]  // name           0
           << QVariant(false)        // switchhog      1
           << QVariant(false)        // team divide    2
           << QVariant(false)        // solid land     3
           << QVariant(false)        // border         4
  ...
  mutant << predefSchemesNames[10]  // name           0
         << QVariant(false)         // switchhog      1
         << QVariant(false)         // team divide    2
         << QVariant(false)         // solid land     3
         << QVariant(false)         // border         4
  ...
  construction << predefSchemesNames[11]  // name           0
               << QVariant(false)         // switchhog      1
               << QVariant(false)         // team divide    2
               << QVariant(false)         // solid land     3
               << QVariant(false)         // border         4
               ...
               << QVariant(100)           // get away time  42
               << QVariant(0)             // world edge     43
               // NOTE: If you change this, also change the defaults in the
               // Construction Mode script
               << QVariant(
                      "initialenergy=550, energyperround=50, maxenergy=1000, "
                      "cratesperround=5")  // scriptparam    44
      ;

  QList<QVariant> specialists;
  specialists << predefSchemesNames[12]  // name           0
              ...
              << QVariant(100)           // get away time  42
              << QVariant(0)             // world edge     43
              // NOTE: If you change this, also change the defaults in the The
              // Specialists script
  ...
  spaceinvasion
      ...
      << QVariant(100)           // get away time  42
      << QVariant(0)             // world edge     43
      // NOTE: If you change this, also change the defaults in the Space
      // Invasion script
      << QVariant(
             "rounds=3, shield=30, barrels=5, pings=2, barrelbonus=3, "
             "shieldbonus=30, timebonus=4")  // scriptparam    44
      ;

  QList<QVariant> hedgeeditor;
  hedgeeditor << predefSchemesNames[14]  // name           0
              << QVariant(false)         // switchhog      1
              << QVariant(false)         // team divide    2
              << QVariant(false)         // solid land     3
              << QVariant(false)         // border         4
  ...
  racer << predefSchemesNames[15]  // name           0
        << QVariant(false)         // switchhog      1
        << QVariant(false)         // team divide    2
        << QVariant(true)          // solid land     3
        << QVariant(true)          // border         4
  ...
  if (!QDir(directory).exists()) {
    ...
    qDebug(
        "No /Schemes/Game directory found. Trying to import game schemes from "
        "schemes.ini.");

    QSettings legacyFileConfig(
        cfgdir.absolutePath() + QStringLiteral("/schemes.ini"),
        QSettings::IniFormat);
    ...
    for (int i = 0; i < size; ++i) {
      ...
      QString schemeName = legacyFileConfig.value(spNames.at(0)).toString();
      if (!schemeName.isNull() &&
          !predefSchemesNamesLower.contains(schemeName.toLower())) {
        QList<QVariant> scheme;
        QFile file(directory + QStringLiteral("/") + schemeName +
                   QStringLiteral(".hwg"));
        ...
        if (file.open(QIODevice::WriteOnly)) {
          ...
          for (int k = 0; k < spNames.size(); ++k) {
            ...
            // File handling
            // We skip the name key (k==0), it is not stored redundantly in
            // file. The file name is used for that already.
            if (k != 0) {
              // The file is just a list of key=value pairs
              stream << spNames[k] << "="
                     << legacyFileConfig.value(spNames[k], defaultScheme[k])
                            .toString()
                     << "\n";
            }
          }
    ...
    for (int i = 0; i < scheme_dir.size(); i++) {
      ...
      if (predefSchemesNamesLower.contains(schemeName.toLower())) {
        qWarning(
            "Game scheme \"%s\" not loaded from file, name collides with a "
            "default scheme!",
...
QVariant GameSchemeModel::headerData(int section, Qt::Orientation orientation,
                                     int role) const {
  Q_UNUSED(section);
...
bool GameSchemeModel::setData(const QModelIndex &index, const QVariant &value,
                              int role) {
  if (!index.isValid() || index.row() < numberOfDefaultSchemes ||
      index.row() >= schemes.size() || index.column() >= defaultScheme.size() ||
      role != Qt::EditRole)
    return false;
...
bool GameSchemeModel::insertRows(int row, int count,
...
bool GameSchemeModel::removeRows(int row, int count,
                                 const QModelIndex &parent) {
  if (count != 1 || row < numberOfDefaultSchemes || row >= schemes.size())
    return false;
  ...
  QFile(cfgdir.absolutePath() + QStringLiteral("/Schemes/Game/") +
        scheme[j].toString() + QStringLiteral(".hwg"))
      .remove();
...
QVariant GameSchemeModel::data(const QModelIndex &index, int role) const {
  if (!index.isValid() || index.row() < 0 || index.row() >= schemes.size() ||
...
void NetGameSchemeModel::setNetSchemeConfig(QStringList cfg) {
  ...
  for (int i = 0; i < cfg.size(); ++i) netScheme[i] = QVariant(cfg[i]);
